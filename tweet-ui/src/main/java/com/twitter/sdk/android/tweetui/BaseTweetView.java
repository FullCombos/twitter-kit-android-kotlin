...
public abstract class BaseTweetView extends AbstractTweetView {
    ...
    private void setStyleAttributes(TypedArray a) {
        ...
        if (isLightBg) {
            photoErrorResId = R.drawable.twitter_ic_tweet_photo_error_light;
            birdLogoResId = R.drawable.twitter_ic_logo_blue;
            retweetIconResId = R.drawable.twitter_ic_retweet_light;
        } else {
            photoErrorResId = R.drawable.twitter_ic_tweet_photo_error_dark;
            birdLogoResId = R.drawable.twitter_ic_logo_white;
            retweetIconResId = R.drawable.twitter_ic_retweet_dark;
        }
        ...
        final Callback<Tweet> repoCb = new Callback<Tweet>() {
            ...
            public void failure(@NonNull TwitterException exception) {
                Twitter.getLogger().d(TAG,
                        String.format(Locale.ENGLISH, TweetUtils.LOAD_TWEET_DEBUG, tweetId), null);
            }
        };
            ...
            avatarView.setOnClickListener(v -> {
                if (tweetLinkClickListener != null) {
                    tweetLinkClickListener.onLinkClick(displayTweet,
                            TweetUtils.getProfilePermalink(displayTweet.user.getScreenName()));
                } else {
                    ...
                    if (!IntentUtils.safeStartActivity(getContext(), intent)) {
                        Twitter.getLogger().e(TweetUi.LOGTAG,
                                "Activity cannot be found to open URL", null);
                    }
                }
