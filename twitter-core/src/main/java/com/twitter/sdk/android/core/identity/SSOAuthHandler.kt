 ...
 */
internal class SSOAuthHandler(
    authConfig: TwitterAuthConfig,
    callback: Callback<TwitterSession>,
    requestCode: Int
    ...
    companion object {
        ...
        private const val DOGFOOD_SIGNATURE =
            "308203523082023aa00302010202044fd0006b300d06092a864" +
         ...
         */
        private fun availableSSOPackage(context: Context): String? {
            return if (checkAppSignature(
                    context,
                ...
                TWITTER_PACKAGE_NAME
            } else if (checkAppSignature(
                    context,
                    DOGFOOD_PACKAGE_NAME,
                    DOGFOOD_SIGNATURE
                )
            ) {
                DOGFOOD_PACKAGE_NAME
        ...
        fun isAvailable(context: Context): Boolean {
            return checkAppSignature(context, TWITTER_PACKAGE_NAME, TWITTER_SIGNATURE) ||
                    checkAppSignature(context, DOGFOOD_PACKAGE_NAME, DOGFOOD_SIGNATURE)
        }
        ...
        private fun checkAppSignature(
            ...
            val signatures = getSignatures(context, packageName).orEmpty()
            for (s in signatures) {
    ...
    private fun startAuthActivityForResult(activity: Activity): Boolean {
        val packageName = availableSSOPackage(activity)
        if (packageName == null) {
